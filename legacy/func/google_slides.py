...
ROUTINE_LAYOUT_ID = os.getenv(
    "ROUTINE_LAYOUT_ID"
)  # ID del layout específico para rutinas

if not TEMPLATE_PRESENTATION_ID:
    raise ValueError(
        "⚠ ERROR: No se encontró TEMPLATE_PRESENTATION_ID en las variables de entorno."
    )
if not ROUTINE_LAYOUT_ID:
    raise ValueError(
        "⚠ ERROR: No se encontró ROUTINE_LAYOUT_ID en las variables de entorno."
    )
...
def create_presentation(routine_data):
    ...
    table_x, table_y = (
        50,
        80,
    )  # La tabla estará debajo del título, centrada horizontalmente
    ...
    for i, rutina in enumerate(routine_data):
        ...
        requests_text.append(
            {"insertText": {"objectId": title_id, "text": f"Día {i + 1}"}}
        )
        ...
        for row, exercise in enumerate(rutina["rutina"], start=1):
            requests_text.append(
                _insert_table_text(table_id, row, 0, exercise["ejercicio"])
            )
            requests_text.append(
                _insert_table_text(table_id, row, 1, exercise["series"])
            )
            requests_text.append(
                _insert_table_text(
                    table_id, row, 2, ", ".join(exercise["repeticiones"])
                )
            )
            ...
            for col in range(num_cols):
                requests_format.append(
                    _format_table_cell(table_id, row, col, row_color)
                )
                # Aplicar texto blanco a las celdas de datos
                requests_format.append(
                    {
                        "updateTextStyle": {
                            "objectId": table_id,
    ...
    try:
        slides_service.presentations().batchUpdate(
            presentationId=presentation_id, body={"requests": requests_text}
        ).execute()
